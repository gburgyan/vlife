...
\begin{document}
    ...
    \begin{abstract}
        ...
        VLife achieves this through a novel nibble-packed cell representation storing both cell liveness and neighbor counts in 4 bits per cell, enabling $O(1)$ rule evaluation. Fixed $32 \times 32$ tiles provide cache-efficient processing and natural parallelization boundaries. Lookup tables eliminate conditional branches, and hierarchical activity masking---including block-level modification tracking via 64-bit masks---enables $O(1)$ skipping of inactive regions. Boundary-aware parallelism achieves full parallel execution with atomic operations only for the $\sim$12\% of cells on tile boundaries. Optional AVX-512 SIMD optimization replaces lookup tables with direct computation for additional throughput on supported hardware.

        Benchmark results demonstrate the activity-proportional property: a single glider processes at 6.17M generations/second while random $256^2$ soup processes at 17.7K gen/sec---a 350$\times$ ratio closely matching the activity ratio. Dense $1024^2$ patterns achieve $\sim$1.4 billion cell-updates per second. An optional metrics collection system with zero runtime overhead validates the $O(k)$ complexity claim empirically. Unlike HashLife, which excels on self-similar patterns but struggles with chaos, VLife provides \emph{predictable, bounded} performance across all pattern types, making it suitable for real-time visualization and chaotic pattern exploration.
    ...
    \begin{enumerate}
        ...
        \item \textbf{Hierarchical skip optimization}: Block-level modification tracking (64-bit mask for 8$\times$8 blocks) combined with activity row masking to bypass dead regions
        ...
        \item \textbf{Platform-specific SIMD}: ARM NEON optimization with prefetching, optional AVX-512 direct computation path
        \item \textbf{Zero-overhead metrics}: Optional instrumentation system for empirical $O(k)$ validation
    \end{enumerate}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \item Changes array: 512 cell pairs $\times$ 2 bits = 1024 bits = 128 bytes
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item Still lifes: $k = 0$, neighbor updates eliminated but Phase 1 tile checking remains $O(T)$
...
leftNeighbors += rightAlive;         // True count
    ...
    This byte-level encoding enables the rule LUT to process \emph{two cells simultaneously} with a single table lookup. The 256-entry LUT is indexed by the full byte value, and the paired cell's contribution to each neighbor count is handled implicitly within the table---no explicit addition is required at runtime. This doubles throughput compared to per-cell evaluation while maintaining the implicit neighbor relationship.
    ...
    VLife implements hierarchical activity tracking at multiple levels to minimize unnecessary computation.

    \subsubsection{Activity Row Tracking}

    Each tile maintains an 8-bit \texttt{activityRows} mask where each bit corresponds to one block row (8 words, covering 4 cell rows). This mask is updated during Phase 1 scanning and cell modifications:
...
inline void markWordActive(uint32_t wordIdx) {
    ...
    Rows with zero activity bits are skipped during Phase 1, while rows that weren't scanned preserve their activity bits to ensure content isn't lost.

    \textbf{Tile-Level:} Completely inactive tiles (\texttt{activityRows == 0}) are excluded from generation processing entirely.

    \subsubsection{Block-Level Modification Tracking}

    A 64-bit \texttt{wasModified} mask tracks which 4$\times$4 cell blocks had neighbor counts updated during Phase 2. The tile is divided into an 8$\times$8 grid of blocks, with each bit representing one block:

    \begin{verbatim}
Block index formula: ((y & 0x1C) << 1) | (x >> 2)

Block layout (8x8 = 64 blocks in 32x32 tile):
+----+----+----+----+----+----+----+----+
|  0 |  1 |  2 |  3 |  4 |  5 |  6 |  7 | Row 0 (cells 0-3)
+----+----+----+----+----+----+----+----+
|  8 |  9 | 10 | 11 | 12 | 13 | 14 | 15 | Row 1 (cells 4-7)
+----+----+----+----+----+----+----+----+
  ...                                       ...
+----+----+----+----+----+----+----+----+
| 56 | 57 | 58 | 59 | 60 | 61 | 62 | 63 | Row 7 (cells 28-31)
+----+----+----+----+----+----+----+----+
    \end{verbatim}

    When a cell changes state in Phase 2, the four corner blocks of its 3$\times$3 affected area are marked via \texttt{markChangeCorners()}. This ensures all cells that could potentially change (due to neighbor count updates) are re-evaluated in the next Phase 1.

    \textbf{Three-Level Skip Hierarchy:} Phase 1 processing uses a hierarchical skip strategy:
    \begin{enumerate}
        \item \textbf{Block-row level:} If no blocks in a row were modified, skip 8 words (4 cell rows)
        \item \textbf{4-word OR:} If \texttt{cells[i] | cells[i+1] | cells[i+2] | cells[i+3] == 0}, skip the group
        \item \textbf{Individual word:} Process only non-zero words within active groups
    \end{enumerate}

    This hierarchical approach was developed after failed experiments with per-row tracking (5\% regression) and per-cell tracking, which added branch misprediction overhead exceeding the savings. Finer granularity also increases memory usage for tracking masks, consuming additional bandwidth and cache capacity. The 4$\times$4 block granularity provides the optimal balance between skip granularity, decision overhead, and memory efficiency.
    ...
    \subsection{Tile Lifecycle Management}

    \subsubsection{Three-Factor Eviction Safety}

    Tiles are dynamically created when cells are set and must be safely evicted when no longer needed to prevent unbounded memory growth. The \texttt{isSafeToEvict()} predicate requires all three conditions:
...
bool isSafeToEvict() const {
    ...
    Each factor protects against a distinct edge case:
    \begin{itemize}
        \item \texttt{liveCount == 0}: No live cells remain (obvious requirement)
        \item \texttt{activityRows == 0}: No non-zero neighbor counts that could trigger births
        \item \texttt{wasModified == 0}: No pending Phase 2 changes propagated from neighbors
    \end{itemize}

    The combination handles subtle scenarios: a tile with \texttt{activityRows != 0} but \texttt{wasModified == 0} contains static content (e.g., a still life's neighbor counts), while a tile with \texttt{activityRows == 0} but \texttt{wasModified != 0} just received neighbor updates and must be processed next generation.

    \subsubsection{Cooldown-Based Eviction}

    To prevent ``flapping'' (tiles being repeatedly created and destroyed at oscillating boundaries), tiles must remain inactive for a cooldown period before eviction:
...
bool decrementCooldown() {
    ...
    This hysteresis prevents performance degradation from patterns that oscillate near tile boundaries. A glider crossing a tile edge, for example, briefly activates the destination tile; the cooldown ensures the source tile isn't evicted before the glider fully departs.

    \subsection{AVX-512 SIMD Optimization}

    VLife includes an optional AVX-512 optimization for Phase 1 that replaces the 256-entry lookup table with direct SIMD computation.

    \subsubsection{Motivation}

    The standard implementation uses a LUT indexed by a byte (representing two cells):

    \begin{lstlisting}
changeBuff |= ruleLUT[(slice >> 56)] << 62;
changeBuff |= ruleLUT[(slice >> 48) & 0xFF] << 60;
// ... 8 lookups per 64-bit word
    \end{lstlisting}

    While cache-efficient for hot data, LUT lookups incur unpredictable memory access patterns that limit instruction-level parallelism. AVX-512's 512-bit registers can process 64 cell pairs simultaneously using pure computation.

    \subsubsection{Implementation}

    The AVX-512 path applies Conway's rules directly using mask operations:

    \begin{lstlisting}
// Load 64 bytes (64 cell pairs)
__m512i data = _mm512_loadu_si512(cellBytes + offset);

// Extract alive flags
__mmask64 left_alive = _mm512_test_epi8_mask(data, mask_left_alive);
__mmask64 right_alive = _mm512_test_epi8_mask(data, mask_right_alive);

// Compute survival: alive AND neighbors in {2,3}
// Compute birth: !alive AND neighbors == 3
// Account for paired cell contribution to neighbor count
__mmask64 left_changes = (left_alive & ~left_survive) |
                          (~left_alive & left_birth);
    \end{lstlisting}

    \subsubsection{Build Configuration}

    AVX-512 support is controlled by CMake options:

    \begin{itemize}
        \item \texttt{-DENABLE\_AVX512=ON}: Enables AVX-512 via SIMDE (portable emulation layer). Works on any CPU for testing.
        \item \texttt{-DAVX512\_NATIVE=ON}: Uses native intrinsics. Requires AVX-512 hardware (Intel Ice Lake+, AMD Zen 4+).
    \end{itemize}

    Runtime dispatch via \texttt{CpuFeatures::hasAVX512Support()} selects the appropriate code path.

    \subsubsection{Expected Performance}

    On AVX-512-capable hardware:
    \begin{itemize}
        \item \textbf{Phase 1}: 2--4$\times$ speedup from eliminating LUT random access
        \item \textbf{Overall}: 1.5--2.5$\times$ speedup (Phase 1 accounts for $\sim$60\% of execution)
    \end{itemize}

    Note: Some Intel CPUs throttle frequency during heavy AVX-512 workloads, which may reduce net benefit.

    \subsection{Parallel Processing Architecture}

    VLife uses Intel TBB for parallel tile processing with a design optimized for minimal synchronization.

    \subsubsection{Parallel Iteration}

    Generation processing uses \texttt{tbb::parallel\_for\_each} with inline activity filtering:

    \begin{lstlisting}
// Phase 1: Parallel with inline skip
...
// Phase 2: All tiles in parallel
tbb::parallel_for_each(tiles.begin(), tiles.end(),
    [](auto& pair) {
        pair.second->runGenerationChanges<true>();
    }
);
    \end{lstlisting}

    \subsubsection{Atomic vs Non-Atomic Paths}

    The \texttt{runGenerationChanges} method is templated on atomicity:
...
void runGenerationChanges();
    ...
    \begin{itemize}
        \item \texttt{UseAtomics=true}: CAS loops for boundary cell updates (required for parallel)
        \item \texttt{UseAtomics=false}: Direct writes (10--20\% faster, used in sequential mode)
    \end{itemize}

    This eliminates atomic overhead when running single-threaded (e.g., for small tile counts or benchmarking).
    ...
    \section{Metrics Collection System}

    VLife includes an optional instrumentation system for validating the activity-proportional complexity claims empirically.

    \subsection{Zero-Overhead Design}

    When \texttt{VLIFE\_METRICS\_ENABLED} is not defined at compile time, all metrics macros expand to no-ops:
    ...
    This ensures zero runtime overhead in production builds while enabling detailed analysis during research.

    \subsection{Key Metrics}

    The system tracks per-generation metrics supporting the $O(k)$ complexity analysis:

    \begin{itemize}
        \item \textbf{$k$ (state changes)}: \texttt{cellsBorn + cellsDied} per generation
        \item \textbf{$N$ (population)}: Total live cells at generation end
        \item \textbf{Activity ratio ($k/N$)}: Core metric validating $O(k)$ vs $O(N)$ scaling
        \item \textbf{Boundary crossings}: Cross-tile neighbor updates ($\sim$12\% of cells)
        \item \textbf{Atomic operations}: CAS operations performed during Phase 2
        \item \textbf{Timing breakdown}: Phase 1 (rule evaluation) and Phase 2 (neighbor updates) in microseconds
    \end{itemize}

    \subsection{Thread-Local Accumulation}

    To avoid atomic contention during parallel processing, counters use thread-local storage:
...
static thread_local ThreadLocalCounters tlCounters;
    ...
    Thread-local counters are merged at phase boundaries via \texttt{VLIFE\_METRICS\_MERGE\_TL()}.

    \subsection{Export Formats}

    Collected metrics can be exported for analysis:

    \begin{itemize}
        \item \textbf{CSV}: Tabular format with one row per generation (pandas/R compatible)
        \item \textbf{JSON}: Structured data with metadata and summary statistics
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Hierarchical Super-Tiling}: $256 \times 256$ super-tiles could improve skip optimization for very sparse regions
        \item \textbf{Rule Generalization}: Extend LUT generation to support arbitrary birth/survival rules (e.g., B36/S23)
        \item \textbf{Distributed Computing}: Tile-based architecture is amenable to message-passing distributed execution across multiple nodes
    \end{itemize}
    ...
    \begin{enumerate}
        \item \textbf{Activity-proportional complexity}: Formal proof that neighbor updates cost $O(8k)$ where $k \ll N$ for typical patterns, yielding order-of-magnitude improvements over population-proportional approaches. An optional zero-overhead metrics system enables empirical validation.
        ...
        \item \textbf{Hierarchical skip optimization}: Block-level modification tracking via 64-bit masks enables $O(1)$ skipping of unmodified regions, with three-level hierarchy (block-row, 4-word OR, individual word) minimizing branch overhead
        \item \textbf{Boundary-aware parallelism}: Full parallel execution without synchronization barriers, using atomic operations only for the $\sim$12\% of cells on tile boundaries. Template-based atomic/non-atomic paths provide 10--20\% speedup for sequential execution.
    ...
    The techniques presented---incremental neighbor tracking, implicit neighbor relationships via cell pairing, block-level modification tracking, cooldown-based tile lifecycle management, and boundary-aware parallelism---are applicable beyond Game of Life to other cellular automata, lattice-based simulations, and grid computations where change locality can be exploited.
...
// Activity and modification tracking
uint8_t activityRows;      // 8 bits: per-row activity
uint64_t wasModified;      // 64 bits: per-block modification
uint8_t cooldownCounter;   // Eviction cooldown (0-4)

// Block index calculation for wasModified mask
// Maps cell (x,y) to one of 64 4x4 blocks
