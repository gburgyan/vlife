...
GameOfLifeView::GameOfLifeView(QWidget *parent) :
    QMainWindow(parent), isRunning(false), viewportX(0), viewportY(0), viewportWidth(100), viewportHeight(100),
    cellSize(5) {
    setupUI();
    ...
    for (uint32_t y = 0; y < viewportHeight; ++y) {
        for (uint32_t x = 0; x < viewportWidth; ++x) {
            if (cells[y * viewportWidth + x] == SimpleGameOfLife::CellState::ALIVE) {
                QGraphicsRectItem *cellItem = scene->addRect(x * cellSize, y * cellSize, cellSize, cellSize,
                                                             QPen(Qt::black), QBrush(Qt::black));
            } else {
                // Draw a light gray grid for dead cells
                scene->addRect(x * cellSize, y * cellSize, cellSize, cellSize, QPen(Qt::lightGray), QBrush(Qt::white));
            }
        }
    }
...
bool GameOfLifeView::eventFilter(QObject *obj, QEvent *event) {
    if (obj == view->viewport() && event->type() == QEvent::MouseButtonPress) {
        ...
        gameOfLife.setCell(cellX, cellY,
                           currentState == SimpleGameOfLife::CellState::ALIVE ? SimpleGameOfLife::CellState::DEAD
